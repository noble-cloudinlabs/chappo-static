...
<table
	...
	<tr>
		...
		<?php endif; ?>
				 ...
				 * @return string
				 */
				echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Item Name', 'ulina' ), $wishlist ) );
                            ?>
                        </th>
		...
		<?php if ( $show_price || $show_price_variations ) : ?>
			...
			<th class="product-price">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'ulina' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_quantity ) : ?>
			...
			<th class="product-quantity">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'ulina' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_stock_status ) : ?>
			...
			<th class="product-stock-status">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Availability', 'ulina' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $enable_drag_n_drop ) : ?>
			...
			<th class="product-arrange">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'ulina' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<td class="product-thumbnail">
						...
						<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
                                                    <img src="<?php echo ulina_attachment_url($product->get_image_id(), 103, 120); ?>" alt="<?php echo esc_attr($product->get_name()); ?>"/>
						</a>
					...
					<td class="product-name">
						...
						<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
							<?php echo ulina_kses( apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ); ?>
						</a>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price">
							...
							<?php
							if ( $show_price ) {
								echo '<div class="pi01Price">'.ulina_kses( $item->get_formatted_product_price() ).'</div>';
							}

							if ( $show_price_variations ) {
								echo '<div class="pi01Price">'.ulina_kses( $item->get_price_variation() ).'</div>';
							}
							?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							 ...
							 * @return string
							 */
							echo 'out-of-stock' === $item->get_stock_status() ? '<span class="wishlist-out-of-stock">' . esc_html( apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'ulina' ) ) ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html( apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'ulina' ) ) ) . '</span>';
							?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && $item->get_date_added() ) :
								...
								echo '<span class="dateadded">' . esc_html( sprintf( esc_html__( 'Added on: %s', 'ulina' ), $item->get_date_added_formatted() ) ) . '</span>';
							endif;
							?>
							...
							<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
								<?php if ( 'select' === $move_to_another_wishlist_type ) : ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'ulina' ); ?></option>
										<?php
								...
								<?php else : ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										 ...
										 * @return string
										 */
										echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'ulina' ) ) );
										?>
									</a>
								<?php endif; ?>
							...
							if ( $repeat_remove_button ) :
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist button" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'ulina' ) ) ); ?>"><?php esc_html_e( 'Remove', 'ulina' ); ?></a>
							<?php endif; ?>
					...
					<?php endif; ?>
						...
						<td class="product-remove">
							<div>
								 ...
								 */
								?>
                                                            <a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'ulina' ) ) ); ?>"><span></span></a>
							</div>
						</td>
					<?php endif; ?>
				</tr>
		...
		<tr>
			 ...
			 */
			?>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'ulina' ), $wishlist ) ); ?></td>
		</tr>
		<?php
	endif;
	...
	if ( ! empty( $page_links ) ) :
		...
		<tr class="pagination-row wishlist-pagination">
			<td colspan="<?php echo esc_attr( $column_count ); ?>">
				<?php echo ulina_kses( $page_links ); ?>
			</td>
		</tr>
	<?php endif ?>
	</tbody>
